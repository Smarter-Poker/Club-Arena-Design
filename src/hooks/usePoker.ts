 ...
 */
export function useActionTimer(timeLimit: number, onTimeout?: () => void) {
 ...
 */
export function usePotOdds(potSize: number, betToCall: number, estimatedEquity?: number) {
  ...
  const shouldCall = estimatedEquity !== undefined ? estimatedEquity > potOddsPercent : undefined;
...
export function useHandHistory(maxEntries = 50) {
  ...
  const addHand = useCallback(
    (entry: Omit<HandHistoryEntry, 'timestamp'>) => {
      setHistory((prev) => {
...
export function useBetSlider(options: {
  ...
  const setPercentOfPot = useCallback(
    (percent: number) => {
  ...
  const setBigBlinds = useCallback(
    (bbs: number) => {
