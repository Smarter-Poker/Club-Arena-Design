...
import { vipService, VIPStatus, VIPFeature, FeatureAccess, FEATURE_PRICING } from '../services/VIPService';
...
export function VIPProvider({ children }: { children: ReactNode }) {
    ...
    const checkVIPStatus = useCallback(async () => {
        ...
        try {
            ...
            setStatus({
                isVIP: false, expiresAt: null, monthlyLimits: {
    ...
    const checkFeature = useCallback(async (feature: VIPFeature): Promise<FeatureAccess> => {
        if (!user?.id) {
            return { hasAccess: false, isVIP: false, needsPurchase: true, diamondCost: FEATURE_PRICING[feature].cost };
        }
        return vipService.checkFeatureAccess(user.id, feature);
    }, [user?.id]);

    const useFeature = useCallback(async (feature: VIPFeature): Promise<{ success: boolean; charged: number }> => {
        if (!user?.id) {
