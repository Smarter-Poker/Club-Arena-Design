...
const BLINDS_PRESETS = [
    ...
    { label: '0.05/0.10', sb: 0.05, bb: 0.10 },
    { label: '0.10/0.25', sb: 0.10, bb: 0.25 },
    { label: '0.25/0.50', sb: 0.25, bb: 0.50 },
    { label: '0.50/1', sb: 0.50, bb: 1 },
    { label: '1/2', sb: 1, bb: 2 },
    { label: '2/5', sb: 2, bb: 5 },
...
export default function TableConfigPage() {
    ...
    const Slider = ({ label, value, onChange, min, max, step = 1, suffix = '', tooltip }: {
        label: string;
        value: number;
    ...
    return (
        <div className="table-config-page">
            ...
            <div className="config-options">
                ...
                <Toggle label="Private Game" value={config.isPrivate} onChange={(v) => updateConfig('isPrivate', v)} />
                <Toggle label="VIP Only" value={config.isVipOnly} onChange={(v) => updateConfig('isVipOnly', v)} />
                <Toggle label="Bomb Pot" value={config.bombPotEnabled} onChange={(v) => updateConfig('bombPotEnabled', v)} tooltip="Enable bomb pot rounds" />
                <Toggle label="Double Board" value={config.doubleBoard} onChange={(v) => updateConfig('doubleBoard', v)} />
                <Toggle label="Triple Board" value={config.tripleBoard} onChange={(v) => updateConfig('tripleBoard', v)} />
                <Toggle label="Pineapple Hold'em" value={config.pineappleHoldem} onChange={(v) => updateConfig('pineappleHoldem', v)} tooltip="3 hole cards, discard 1" />
                <Toggle label="Seven-Deuce" value={config.sevenDeuceEnabled} onChange={(v) => updateConfig('sevenDeuceEnabled', v)} tooltip="Bonus for winning with 7-2" />
                <Toggle label="NIT Game" value={config.nitGame} onChange={(v) => updateConfig('nitGame', v)} tooltip="Penalty for tight play" />
                <Toggle label="Anonymous Table" value={config.isAnonymous} onChange={(v) => updateConfig('isAnonymous', v)} />
                <Toggle label="Cap" value={config.capEnabled} onChange={(v) => updateConfig('capEnabled', v)} tooltip="Cap the max bet" />
                <Toggle label="Ban Chat" value={config.banChat} onChange={(v) => updateConfig('banChat', v)} />
                <Toggle label="Label as NEW" value={config.labelAsNew} onChange={(v) => updateConfig('labelAsNew', v)} tooltip="Show NEW badge" />
                <Toggle label="Featured Table" value={config.isFeatured} onChange={(v) => updateConfig('isFeatured', v)} tooltip="Feature at top of list" />
                <Toggle label="No Rathole" value={config.noRathole} onChange={(v) => updateConfig('noRathole', v)} tooltip="Prevent leaving with winnings" />
                ...
                <Slider
                    ...
                    min={2} max={10} suffix=" max"
                />

                <Toggle label="Calltime" value={config.calltimeEnabled} onChange={(v) => updateConfig('calltimeEnabled', v)} tooltip="Shot clock for action" />

                <Slider
                    label="Action Time"
                    value={config.actionTimeSeconds}
                    onChange={(v) => updateConfig('actionTimeSeconds', v)}
                    min={5} max={60} suffix=" sec"
                />

                {/* Blinds Slider */}
                <div className="config-slider">
                    <div className="slider-header">
                        <span className="slider-label">Blinds: {config.smallBlind}/{config.bigBlind}</span>
                    </div>
                    <div className="slider-track-container">
                        <input
                            type="range"
                ...
                <div className="config-slider">
                    <div className="slider-header">
                        <span className="slider-label">Buy-in: {config.minBuyInBB} - {config.maxBuyInBB}</span>
                    </div>
                ...
                <Slider
                    ...
                    min={0} max={5} step={0.5} suffix=" Big Blind"
                />
                ...
                <Slider
                    ...
                    min={2} max={10} suffix=" players"
                />

                {/* SECTION: Auto Settings */}
                <Toggle label="Auto Extension" value={config.autoExtension} onChange={(v) => updateConfig('autoExtension', v)} tooltip="Extend table automatically" />
                <Toggle label="Auto Restart" value={config.autoRestart} onChange={(v) => updateConfig('autoRestart', v)} />
                <Toggle label="Auto Create Table" value={config.autoCreateTable} onChange={(v) => updateConfig('autoCreateTable', v)} tooltip="Create new table when full" />
                <Toggle label="Auto UTG Straddle" value={config.autoUtgStraddle} onChange={(v) => updateConfig('autoUtgStraddle', v)} tooltip="Automatic UTG straddle" />
                <Toggle label="Voluntary Straddle" value={config.voluntaryStraddle} onChange={(v) => updateConfig('voluntaryStraddle', v)} tooltip="Allow voluntary straddle" />
                <Toggle label="Insurance" value={config.insuranceEnabled} onChange={(v) => updateConfig('insuranceEnabled', v)} tooltip="All-in insurance option" />
                ...
                {(config.gameMode === 'sng' || config.gameMode === 'mtt') && (
                    <>
                        ...
                        <div className="config-toggle">
                            <span className="toggle-label">
                                Payout Structure
                                <span className="tooltip-icon" title="Prize distribution">?</span>
                            </span>
                            <select
                ...
                {config.gameMode === 'mtt' && (
                    <>
                        ...
                        <Slider
                            ...
                            min={0.5} max={3} step={0.5} suffix="x"
                        />
                        <Toggle
                            label="Custom Add-on"
                        ...
                        <div className="config-slider">
                            <div className="slider-header">
                                <span className="slider-label">Player Number: {config.minPlayers} - {config.maxPlayersRange}</span>
                            </div>
                            <div className="buyin-sliders">
                                <input
                                    type="range"
                ...
                <Slider
                    ...
                    min={1} max={10} suffix=" x Big Blind"
                ...
                <Slider
                    ...
                    min={1} max={100} suffix={config.agentDownlineLimit === null ? '' : ''}
                />

                <Toggle label="Buy-in Authorization" value={config.buyInAuthorization} onChange={(v) => updateConfig('buyInAuthorization', v)} />
                <Toggle label="Restrict Device" value={config.restrictDevice} onChange={(v) => updateConfig('restrictDevice', v)} tooltip="One device per player" />
                <Toggle label="Restrict Observers" value={config.restrictObservers} onChange={(v) => updateConfig('restrictObservers', v)} />
                <Toggle label="GPS Restriction" value={config.gpsRestriction} onChange={(v) => updateConfig('gpsRestriction', v)} />
                <Toggle label="IP Restriction" value={config.ipRestriction} onChange={(v) => updateConfig('ipRestriction', v)} />
                <Toggle label="PC Emulator Restriction" value={config.pcEmulatorRestriction} onChange={(v) => updateConfig('pcEmulatorRestriction', v)} />
                <Toggle label="Photo Rotation Verification" value={config.photoRotationVerification} onChange={(v) => updateConfig('photoRotationVerification', v)} tooltip="Verify identity with photo" />
                <Toggle label="Hide Club Name" value={config.hideClubName} onChange={(v) => updateConfig('hideClubName', v)} />

                <Slider
                    label="Game Length"
                    value={config.gameLengthHours}
                    onChange={(v) => updateConfig('gameLengthHours', v)}
                    min={1} max={24} suffix=" hour"
                />
            </div>
