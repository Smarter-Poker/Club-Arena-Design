...
function getCtx(): AudioContext | null {
    try {
        ...
        if (_ctx.state === 'suspended') {
            _ctx.resume().catch(() => { /* will resume on next user gesture */ });
        }
        return _ctx;
    } catch {
...
export const PremiumSFX = {
    ...
    cardFlip: () => {
        if (!isEnabled()) return;
        playTone(NOTE.C5, 0.12, 0.10, 'sine');
        playTone(NOTE.E5, 0.10, 0.08, 'sine', 0.04);
    },
    ...
    doubleTap: () => {
        ...
        playTone(NOTE.G4, 0.20, 0.06, 'sine', 0.04);
    },
    ...
    dragDrop: () => {
        if (!isEnabled()) return;
        playTone(NOTE.C4, 0.15, 0.10, 'sine');
        playNoise(0.06, 0.05, 800);
        playTone(NOTE.G4, 0.10, 0.06, 'sine', 0.05);
    },
    ...
    ctaClick: () => {
        if (!isEnabled()) return;
        playTone(NOTE.G5, 0.10, 0.08, 'sine');
        playTone(NOTE.E5, 0.10, 0.07, 'sine', 0.06);
    ...
    toggleOn: () => {
        ...
        playTone(NOTE.C5, 0.12, 0.10, 'sine');
        playTone(NOTE.G5, 0.15, 0.10, 'sine', 0.08);
    },
