...
class TournamentRecurringService {
  ...
  start(): void {
    if (this.isRunning) {
      console.debug('[TournamentRecurring] Already running');
      return;
    }

    this.isRunning = true;
    console.debug(
      '[TournamentRecurring] Service started — checking tournaments every 5 min, SNGs every 15 min, Spins every 10 min'
    );
  ...
  stop(): void {
    if (!this.isRunning) {
      console.debug('[TournamentRecurring] Already stopped');
      return;
    }
    ...
    console.debug('[TournamentRecurring] Service stopped');
  }
  ...
  private async checkAndLaunchTournaments(): Promise<void> {
    try {
      ...
      if (!block) {
        console.debug(`[TournamentRecurring] No tournaments scheduled for hour ${hour}`);
        return;
      }

      console.debug(`[TournamentRecurring] Checking tournaments for hour ${hour}`);
      ...
      for (const config of block.tournaments) {
        ...
        if (existing > 0) {
          console.debug(
            `[TournamentRecurring] Tournament "${config.name}" already active (${existing} running)`
          );
          continue;
        }
  ...
  private async checkAndLaunchSNGs(): Promise<void> {
    try {
      ...
      if (activeCount >= threshold) {
        console.debug(
          `[TournamentRecurring] SNGs OK: ${activeCount} active (threshold: ${threshold})`
        );
        return;
      }

      console.debug(`[TournamentRecurring] SNG count low (${activeCount}), launching more...`);
      ...
      console.debug(`[TournamentRecurring] Launched ${launched} SNGs`);
    } catch (err: any) {
  ...
  private async checkAndLaunchSpins(): Promise<void> {
    try {
      ...
      if (activeCount >= threshold) {
        console.debug(
          `[TournamentRecurring] Spins OK: ${activeCount} active (threshold: ${threshold})`
        );
        return;
      }

      console.debug(`[TournamentRecurring] Spin count low (${activeCount}), launching more...`);
      ...
      console.debug(`[TournamentRecurring] Launched ${launched} Spins`);
    } catch (err: any) {
  ...
  private async createTournament(
    ...
    try {
      ...
      // Register horses
      const registered = await this.registerHorsesForTournament(
        tournament.id,
