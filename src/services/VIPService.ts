...
export const FEATURE_PRICING: Record<VIPFeature, {
    cost: number;
    usageType: 'per_use' | 'per_session' | 'permanent';
    description: string;
}> = {
    rabbit_hunt: { cost: 5, usageType: 'per_use', description: 'See what cards would have come' },
    show_stack_bb: { cost: 5, usageType: 'per_session', description: 'Display stack in big blinds' },
    offline_protection: { cost: 10, usageType: 'per_session', description: 'Protection from timeout' },
    auto_time_bank: { cost: 10, usageType: 'per_session', description: 'Automatic time bank usage' },
    time_bank_seconds: { cost: 1, usageType: 'per_use', description: 'Extra 10 seconds of time bank' },
    theme_unlock: { cost: 25, usageType: 'permanent', description: 'Unlock table theme' },
    club_creation: { cost: 100, usageType: 'permanent', description: 'Create additional club' },
    emoji_pack: { cost: 1, usageType: 'permanent', description: 'Unlock 50 emojis' },
    tag_pack: { cost: 1, usageType: 'permanent', description: 'Unlock 100 player tags' }
};
...
class VIPServiceClass {
    ...
    async checkVIPStatus(userId: string): Promise<VIPStatus> {
        ...
        const isVIP = data.is_vip && (!data.vip_expires_at || new Date(data.vip_expires_at) > new Date());
     ...
     */
    async useFeature(userId: string, feature: VIPFeature): Promise<{ success: boolean; charged: number }> {
     ...
     */
    async purchaseFeature(userId: string, feature: VIPFeature, quantity: number = 1): Promise<{ success: boolean; charged: number; error?: string }> {
        const { data, error } = await supabase
            .rpc('fn_purchase_feature', {
                p_user_id: userId,
                p_feature: feature,
                p_quantity: quantity
            });
    ...
    private async getMonthlyUsage(userId: string): Promise<VIPMonthlyLimits> {
        ...
        return {
            ...
            timeBankSeconds: { used: usage['time_bank_seconds'] || 0, limit: VIP_GOLD_LIMITS.timeBankSeconds },
    ...
    private async checkVIPQuota(userId: string, feature: VIPFeature): Promise<boolean> {
        ...
        if (['rabbit_hunt', 'show_stack_bb', 'offline_protection', 'auto_time_bank'].includes(feature)) {
            return true;
        }
    ...
    private async consumeVIPQuota(userId: string, feature: VIPFeature): Promise<void> {
        ...
        // Increment count
        await supabase.rpc('fn_increment_vip_usage', {
            p_user_id: userId,
            p_feature: feature
        });
    }
    ...
    private async consumePurchase(userId: string, feature: VIPFeature): Promise<void> {
        await supabase.rpc('fn_consume_feature_use', {
            p_user_id: userId,
            p_feature: feature
        });
    }
