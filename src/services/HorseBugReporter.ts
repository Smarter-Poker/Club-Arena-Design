...
class HorseBugReporterService {
    ...
    startCapturing(): void {
        ...
        console.log('[HorseBugReporter] Mini-agent QA system ACTIVE — capturing bugs');
    }
    ...
    validateChips(
        ...
        if (isNaN(stackAfter) || isNaN(stackBefore)) {
            this.report({
                horseName, horseId, tableId, tableName, handNumber,
    ...
    validatePot(
        ...
        if (isNaN(potAmount)) {
            this.report({
                horseName: 'PotValidator', horseId: 'system', tableId, tableName, handNumber,
    ...
    getReports(filter?: { severity?: BugSeverity; category?: BugCategory; resolved?: boolean }): BugReport[] {
    ...
    getStats(): {
        ...
        const bySeverity: Record<BugSeverity, number> = { critical: 0, high: 0, medium: 0, low: 0, info: 0 };
