...
const POY_WEBHOOK_URL = import.meta.env.VITE_POY_WEBHOOK_URL || 'https://smarter.poker/api/club-arena/results';
const POY_API_KEY = import.meta.env.VITE_POY_API_KEY || '';
const POY_LEADERBOARD_URL = import.meta.env.VITE_POY_LEADERBOARD_URL || 'https://smarter.poker/api/poy/leaderboard';
...
export const POYService = {
     ...
     */
    async submitTournamentResult(data: TournamentResultPayload): Promise<{ success: boolean; points_awarded?: number }> {
        if (!POY_API_KEY) {
     ...
     */
    async submitCashSession(data: CashSessionPayload): Promise<{ success: boolean; points_awarded?: number }> {
        if (!POY_API_KEY) {
            console.warn('POYService: No API key configured, skipping submission');
            return { success: false };
        }
