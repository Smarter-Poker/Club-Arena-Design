...
export function countOuts(holeCards: string[], board: string[]): number {
  ...
  const uniqueRanks = [...new Set(ranks.map(r => rankValues[r] || 0))].sort((a,b) => a-b).filter(r => r > 0);
...
export function getPositionColor(tier: string): string {
  switch (tier) {
    case 'utg': return '#ef4444'; // red
    case 'mid': return '#f59e0b'; // amber
    case 'late': return '#10b981'; // green
