...
class TableBalancerClass {
  ...
  calculateMoves(tables: BalancerTable[]): MoveInstruction[] {
    ...
    // Target: some tables get idealPerTable+1, others get idealPerTable
    // Sort tables to assign targets: tables with most players get higher targets first
    const tableTargets = tables
      .map((t, i) => ({
        ...t,
        index: i,
        target: i < remainder ? idealPerTable + 1 : idealPerTable,
      }))
      .sort((a, b) => b.playerCount - a.playerCount);

    // Assign targets based on player count (most populated get the +1 targets)
    const sortedByCount = [...tableTargets].sort((a, b) => b.playerCount - a.playerCount);
...
export const tableBalancer = new TableBalancerClass();
export default tableBalancer;
