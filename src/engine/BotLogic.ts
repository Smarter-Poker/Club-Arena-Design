...
const STYLE_PARAMS: Record<HorseStyle, StyleParams> = {
    tag: {
        ...
        cbetFreq: 0.70,          // High cbet
        bluffFreq: 0.12,         // Selective bluffs
        slowplayFreq: 0.10,      // Rarely slowplays
    ...
    lag: {
        ...
        threeBetThreshold: 0.60, // Wider 3-bets
    ...
    balanced: {
        vpipThreshold: 0.30,     // ~25% VPIP
        pfrThreshold: 0.40,      // Standard opens
        threeBetThreshold: 0.70, // Solid 3-bets
        cbetFreq: 0.65,          // Mixed cbet
        bluffFreq: 0.18,         // Balanced bluffs
        slowplayFreq: 0.20,      // Mixed slowplays
    ...
    tricky: {
        ...
        bluffFreq: 0.20,         // Good bluff frequency
    ...
    grinder: {
        ...
        cbetFreq: 0.60,          // Moderate cbet
        bluffFreq: 0.10,         // Low bluffs (value-heavy)
        slowplayFreq: 0.12,      // Rarely slowplays
        checkRaiseFreq: 0.10,    // Selective c/r
...
export class BotLogic {
    ...
    static decide(
        ...
        const handStrength = this.calculateHandStrength(
            player.cards, communityCards, stage, gameState.gameVariant
        );
    ...
    static calculateHandStrength(
        ...
        const rankingScores: Record<number, [number, number]> = {
            1: [0.05, 0.20],  // High Card
            2: [0.20, 0.45],  // One Pair
            3: [0.50, 0.65],  // Two Pair
    ...
    private static evaluateHoleCards(cards: Card[]): number {
        ...
        else if (gap === 2) score -= 1;  // One-gapper
    ...
    private static evaluateOmahaHoleCards(cards: Card[]): number {
        ...
        for (const [r, count] of rankCounts) {
            if (count === 2 && r >= 10) score += 3; // High pair
