...
export const OFCPineappleEngine = {
   ...
   */
  placeCard(game: OFCGameState, playerId: string, cardIndex: number, row: OFCRow): OFCGameState {
    const newGame = { ...game };
  ...
  evaluateHand(cards: OFCCard[]): { rank: OFCHandRank; strength: number } {
    ...
    if (isStraight && isFlush) {
      ...
      handRank = 'full_house';
      strength =
        7000 + this.getCountRankValue(rankCounts, 3) * 100 + this.getCountRankValue(rankCounts, 2);
    } else if (isFlush) {
      handRank = 'flush';
   ...
   */
  calculateRoyalties(hand: OFCHand): {
    front: number;
    middle: number;
    back: number;
    total: number;
    ...
    if (frontEval.rank === 'trips') {
      ...
      const tripRank =
        [...rankCounts.entries()].find(([_, count]) => count === 3)?.[0] || hand.front[0].rank;
