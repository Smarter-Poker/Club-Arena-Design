 ...
 * - Periodic summary via ENGINE_TELEMETRY bus event
 */
...
class EngineTelemetryClass {
  ...
  /**
   * Emit telemetry to bus.
   */
  private emitSnapshot(): void {
    const snapshot = this.getSnapshot();
    if (snapshot.global.activeTables > 0) {
      masterBus.emit('ENGINE_TELEMETRY', {
...
export const engineTelemetry = new EngineTelemetryClass();
