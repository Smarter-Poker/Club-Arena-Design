...
class AtomicStackServiceClass {
  ...
  atomicDebit(
    ...
    if (sv.version !== expectedVersion) {
      masterBus.emit('STACK_RACE_DETECTED', {
        tableId,
        userId,
        operation: 'debit',
        expectedVersion,
  ...
  atomicSettle(tableId: string, settlements: StackSettlement[]): BatchSettlementResult {
    ...
    masterBus.emit('STACK_SETTLEMENT', {
      tableId,
...
export const atomicStackService = new AtomicStackServiceClass();
export default atomicStackService;
