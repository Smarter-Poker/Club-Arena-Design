 ...
 * - Countdown warning (30 seconds) before break
 * - Bus emissions for all phases
 */
...
class TableBreakEngineClass {
  ...
  async initiateBreak(
    ...
    masterBus.emit('TABLE_BREAK_STARTED', {
      tableId: brokenTable.tableId,
      tournamentId,
      playerCount: brokenTable.playerCount,
    });
    ...
    masterBus.emit('TABLE_BREAK_COMPLETED', {
      tableId: brokenTable.tableId,
      tournamentId,
      totalMoved: movements.length,
...
export const tableBreakEngine = new TableBreakEngineClass();
export default tableBreakEngine;
