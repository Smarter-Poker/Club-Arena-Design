...
const VARIANT_DISPLAY: Record<string, { label: string; sub?: string; css: string }> = {
    ...
    ofc_pineapple:   { label: 'PINEAP',            css: 'ofc_pineapple' },
    pineapple:       { label: 'PINEAP',            css: 'pineapple' },
    crazy_pineapple: { label: 'C PINE',            css: 'crazy_pineapple' },
...
function getTourneyTypeLabel(type: string): string {
    switch (type) {
        case 'ko': return 'KO';
        case 'pko': return 'PKO';
        case 'mystery': return '?KO';
        case 'freeroll': return 'FREE';
        case 'turbo': return '⚡';
        default: return 'FO';
    }
}
        ...
        <div className="table-card-wrapper" style={{ position: 'relative' }}>
            ...
            {isAdmin && onDelete && (
                <button
                    ...
                    onClick={(e) => { e.preventDefault(); e.stopPropagation(); onDelete(table.id); }}
                    title="Delete table"
                >
                    ✕
                </button>
...
export function TournamentCard({ tournament }: TournamentCardProps) {
    ...
    return (
        <Link to={`/tournaments/${tournament.id}`} className={`dgc dgc--${variant.css} dgc--mtt`}>
            ...
            {/* Players */}
            <div className="dgc__players">
                <span className={`dgc__players-dot ${tournament.current_players > 0 ? '' : 'dgc__players-dot--empty'}`} />
...
export function SNGCard({ tournament }: TournamentCardProps) {
    ...
    return (
        <Link to={`/tournaments/${tournament.id}`} className={`dgc dgc--sng`}>
            ...
            {!isHU && (
                <div className="dgc__max-badge">
                    {tournament.max_players <= 6 ? '6 Max' : '9 Max'}
                </div>
            )}
