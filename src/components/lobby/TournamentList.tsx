    ...
    <div className="tournament-row" onClick={onViewDetails}>
      ...
      <div className="tournament-row__info">
        <div className="tournament-row__header">
          <span
            className={`tournament-row__type tournament-row__type--${tournament.type.toLowerCase()}`}
          >
            {tournament.type}
          </span>
        ...
        <div className="tournament-row__meta">
          ...
          <span className="tournament-row__structure">
            {tournament.blindLevelMinutes}min levels
          </span>
          {tournament.clubName && (
            <span className="tournament-row__club">• {tournament.clubName}</span>
          )}
        </div>
      </div>
      ...
      <div className="tournament-row__time">
        ...
        <span className="tournament-row__start-time">
          {formatTime(new Date(tournament.startTime))}
        </span>
      </div>

      {/* Buy-In */}
      <div className="tournament-row__buyin">
        <span className="tournament-row__buyin-total">{formatAmount(totalBuyIn, currency)}</span>
        {tournament.fee > 0 && (
      ...
      <div className="tournament-row__prize">
        <span
          className={`tournament-row__prize-amount ${isGuaranteed ? 'tournament-row__prize-amount--gtd' : ''}`}
        >
      ...
      <div className="tournament-row__players">
        ...
        <span className="tournament-row__status" style={{ color: statusColor }}>
...
export function TournamentList({
  ...
  const groupedTournaments = useMemo(() => {
    ...
    const registering = tournaments.filter((t) =>
      ['scheduled', 'registering', 'late_reg'].includes(t.status)
    );
          ...
          {groupedTournaments.registering.map((tournament, idx) => (
            <div
              ...
              style={{
                ...
                transform: visibleTournaments.has(idx + groupedTournaments.running.length)
                  ? 'translateY(0)'
                  : 'translateY(8px)',
                transition: 'all 0.35s cubic-bezier(0.175, 0.885, 0.32, 1.275)',
              }}
