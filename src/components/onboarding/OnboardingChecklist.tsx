    ...
    return (
        <div className="onboarding-checklist">
            <div className="checklist-header">
                ...
                <span className="checklist-progress">{completedCount}/{items.length}</span>
            </div>
                        ...
                        {item.reward && (
                            <div className="item-reward">
                                <span className="reward-icon">
                                    {item.reward.type === 'diamonds' ? '' : ''}
                                </span>
                                <span className="reward-amount">+{item.reward.amount}</span>
                                {item.completed && (
                                    <button
                                        className="claim-btn"
                                        onClick={(e) => { e.stopPropagation(); onClaimReward?.(item.id); }}
                                    >
                                        Claim
                                    </button>
                                )}
            ...
            {completedCount === items.length && (
                <div className="all-complete">
                     All tasks complete! You're ready to dominate.
                </div>
            )}
        </div>
    );
