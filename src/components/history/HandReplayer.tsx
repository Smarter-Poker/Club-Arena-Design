            ...
            <div className="replayer" onClick={e => e.stopPropagation()} style={{ opacity: mounted ? 1 : 0, transform: mounted ? 'translateY(0)' : 'translateY(8px)', transition: 'all 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275)' }}>
                ...
                <div className="replayer__table">
                    <div className="community-cards">
                        ...
                        {Array(5 - visibleCards.length).fill(0).map((_, idx) => (
                            <div key={`empty-${idx}`} className="card empty" />
                        ))}
                    </div>

                    <div className="pot">
                        Pot: {handData.potSize.toLocaleString()}
                    </div>
                </div>
                ...
                <div className="replayer__controls">
                    ...
                    <button onClick={() => setCurrentActionIndex(Math.max(0, currentActionIndex - 1))}>⏪</button>
