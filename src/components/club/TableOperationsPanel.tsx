...
const getStatusBadgeStyle = (status: string): React.CSSProperties => ({
    ...
    background: status === 'running' ? 'rgba(34,197,94,0.15)' :
                status === 'paused' ? 'rgba(234,179,8,0.15)' :
                status === 'waiting' ? 'rgba(59,130,246,0.15)' :
                'rgba(255,255,255,0.1)',
    color: status === 'running' ? '#22c55e' :
           status === 'paused' ? '#eab308' :
           status === 'waiting' ? '#3b82f6' : '#888',
});
...
export default function TableOperationsPanel({ clubId }: Props) {
    ...
    const handleKickPlayer = async () => {
        ...
        await tableService.kickPlayer(confirmAction.tableId, confirmAction.userId, 'Removed by admin');
    ...
    // ─── Format helpers ────────────────────────────────────────────────────────
    const formatChips = (n: number) => n.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
    const formatVariant = (v: string) => {
        const map: Record<string, string> = { nlh: "NL Hold'em", plo: 'PLO', plo5: 'PLO-5', nlh_plo: 'Mixed', short_deck: 'Short Deck' };
                ...
                return (
                    ...
                    >
                        ...
                        <div style={styles.tableHeader} onClick={() => toggleExpand(table.id)}>
                            ...
                            <span style={getStatusBadgeStyle(table.status)}>
                                {table.status}
                            </span>
                        </div>
                        ...
                        {confirmAction.type === 'kick' ? (
                            <>
                                ...
                                <div style={styles.confirmText}>
                                    Remove <strong>{confirmAction.playerName}</strong> from the table?
                                    Their chips will be returned to their wallet.
                                </div>
                                <div style={styles.confirmActions}>
                                    <button style={styles.cancelBtn} onClick={() => setConfirmAction(null)}>Cancel</button>
                                    <button style={styles.dangerBtn} onClick={handleKickPlayer}>Kick Player</button>
                                </div>
                            </>
                        ) : (
                            <>
                                ...
                                <div style={styles.confirmText}>
                                    This will close the table and all seated players will be cashed out.
                                    This action cannot be undone.
                                </div>
                                <div style={styles.confirmActions}>
                                    <button style={styles.cancelBtn} onClick={() => setConfirmAction(null)}>Cancel</button>
                                    <button style={styles.dangerBtn} onClick={handleCloseTable}>Close Table</button>
                                </div>
                            </>
                        )}
                    </div>
