    ...
    return (
        <div className="waitlist-manager">
            <div className="waitlist-header">
                ...
                <div className="seat-status">
                    <span className={seatsAvailable > 0 ? 'available' : 'full'}>
                        {seatsAvailable > 0 ? `${seatsAvailable} seat${seatsAvailable > 1 ? 's' : ''} available` : 'Table Full'}
                    </span>
                </div>
            </div>
                    ...
                    {waitlist.map((entry, idx) => (
                        ...
                        >
                            ...
                            {isDealer && seatsAvailable > 0 && idx === 0 && (
                                <button
                                    className="seat-btn"
                                    onClick={() => handleSeatPlayer(entry)}
                                >
                                    Seat
                                </button>
                            )}
                            {(isDealer || entry.userId === currentUserId) && (
                                <button
                                    ...
                                    onClick={() => entry.userId === currentUserId ? handleLeave() : handleRemove(entry)}
                                >
                                    ×
                                </button>
                            )}
            ...
            {!isInWaitlist && currentUserId && (
                ...
                >
                    {seatsAvailable > 0 ? 'Seats Available - Join Table' :
            ...
            {isInWaitlist && (
                <div className="my-position">
                    ...
                    <button className="leave-btn" onClick={handleLeave}>Leave Waitlist</button>
                </div>
            )}
        </div>
    );
