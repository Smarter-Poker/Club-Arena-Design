...
const RebuyModal: React.FC<RebuyModalProps> = ({
    isOpen, rebuyCost, rebuyChips, walletBalance, onConfirm, onClose, isProcessing
}) => {
        ...
        <div className="rebuyModalOverlay" onClick={onClose}>
            <div className="rebuyModalContent" onClick={e => e.stopPropagation()}>
                <div className="rebuyModalHeader">
                    <h3>Rebuy</h3>
                    <button className="rebuyCloseBtn" onClick={onClose}>×</button>
                </div>
                <div className="rebuyModalBody">
                    <div className="rebuyInfo">
                        ...
                        <div className="rebuyRow">
                            ...
                            <span className={`rebuyValue ${!canAfford ? 'insufficient' : ''}`}>{walletBalance}</span>
                        </div>
                    </div>
                    {!canAfford && (
                        <div className="rebuyWarning">Insufficient balance for rebuy</div>
                    )}
                </div>
                <div className="rebuyModalFooter">
                    <button className="rebuyDeclineBtn" onClick={onClose}>Decline</button>
                    <button
