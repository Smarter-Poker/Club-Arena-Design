  ...
  if (orientation === 'circular') {
    ...
    return (
      <div className={containerClasses}>
        <svg className="timer-bar__circle" viewBox={`0 0 ${(radius + 4) * 2} ${(radius + 4) * 2}`}>
          {/* Background circle */}
          <circle className="timer-bar__circle-bg" cx={radius + 4} cy={radius + 4} r={radius} />
        ...
        {showCountdown && <span className="timer-bar__countdown">{formattedTime}</span>}
      </div>
    );
  }
    ...
    <div className={containerClasses}>
      ...
      {showCountdown && (
        <div className="timer-bar__info">
          ...
          {timeBank > 0 && (
            <span
              className={`timer-bar__time-bank ${isUsingTimeBank ? 'timer-bar__time-bank--active' : ''} ${timeBankFlash ? 'timer-bar__time-bank--flash' : ''}`}
            >
              +{timeBank}s
            </span>
          )}
