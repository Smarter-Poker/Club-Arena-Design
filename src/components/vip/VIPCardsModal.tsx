...
const FEATURES = [
    ...
    { key: 'time_bank_seconds', label: 'Free Time Bank', vipValue: `${VIP_GOLD_LIMITS.timeBankSeconds}s` },
    { key: 'theme_unlock', label: 'Available Themes', vipValue: `${VIP_GOLD_LIMITS.themes}` },
    { key: 'club_creation', label: 'Club Creation Limit', vipValue: `${VIP_GOLD_LIMITS.clubCreation}` },
    { key: 'emoji_pack', label: 'Free Emojis', vipValue: `${VIP_GOLD_LIMITS.emojis}` },
    { key: 'tag_pack', label: 'Tags', vipValue: `${VIP_GOLD_LIMITS.tags}` },
    { key: 'leaderboard', label: 'Score Leaderboard Boost', vipValue: `${(VIP_GOLD_LIMITS.leaderboardBoost * 100).toFixed(0)}%` }
] as const;
        ...
        <div className="vip-modal-overlay" onClick={onClose}>
            <div className="vip-modal" onClick={e => e.stopPropagation()}>
                ...
                <div className="vip-modal__header">
                    <h2> VIP GOLD</h2>
                    <button className="vip-modal__close" onClick={onClose}>×</button>
                </div>
                                ...
                                return (
                                    <tr key={feature.key}>
                                        ...
                                        <td className="feature-cost">
                                            {pricing ? `${pricing.cost}/${pricing.usageType.replace('per_', '').replace('_', ' ')}` : '-'}
                                        </td>
                                    </tr>
                                );
                            })}
