...
export function Tabs({
    ...
    return (
        <TabsContext.Provider value={{ activeTab, setActiveTab: handleTabChange }}>
            <div className={`tabs tabs-${variant} tabs-${size} ${fullWidth ? 'tabs-full' : ''} ${className}`}>
                {children}
            </div>
...
export function SimpleTabs({
    ...
    tabs: Array<{ id: string; label: string; content: React.ReactNode; icon?: React.ReactNode; disabled?: boolean }>;
    defaultTab?: string;
    onChange?: (tabId: string) => void;
    variant?: 'default' | 'pills' | 'underline' | 'enclosed';
    className?: string;
    ...
    return (
        <Tabs defaultTab={defaultTab || tabs[0]?.id} onChange={onChange} variant={variant} className={className}>
            <TabList>
                {tabs.map((tab) => (
