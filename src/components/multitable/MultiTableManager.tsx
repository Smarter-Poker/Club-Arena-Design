    ...
    return (
        <div className="multi-table-manager">
            ...
            <div className="table-tabs">
                    ...
                    >
                        ...
                        {table.isMyTurn && (
                            <span className="turn-indicator">
                                {table.timeRemaining && table.timeRemaining < 10
                        ...
                        <button
                            className="close-tab"
                            onClick={(e) => { e.stopPropagation(); onTableClose(table.id); }}
                        >
                            ✕
                        </button>
                    </div>
                ...
                {tables.length < 4 && (
                    <button className="add-table-tab">
                        + Add Table
                    </button>
                )}
            </div>
...
export const useMultiTable = () => {
    ...
    const addTable = (table: ActiveTable) => {
        if (tables.length >= 4) {
            alert('Maximum 4 tables allowed');
            return false;
        }
    ...
    useEffect(() => {
        const urgentTable = tables.find(
            (t) => t.isMyTurn && t.timeRemaining && t.timeRemaining < 5
        );
