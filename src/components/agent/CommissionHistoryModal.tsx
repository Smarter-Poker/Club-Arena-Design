    ...
    const loadCommissions = async () => {
        ...
        try {
            ...
            if (!error && data) {
                ...
                // Calculate summary
                const totalEarned = data.filter(e => e.type === 'rake').reduce((sum, e) => sum + e.amount, 0);
                const totalPaid = data.filter(e => e.type === 'payout').reduce((sum, e) => sum + Math.abs(e.amount), 0);
                ...
                const lastMonth = data
                    .filter(e => new Date(e.created_at) >= lastMonthStart && new Date(e.created_at) < thisMonthStart && e.type === 'rake')
                    .reduce((sum, e) => sum + e.amount, 0);
