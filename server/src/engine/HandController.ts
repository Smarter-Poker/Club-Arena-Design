...
import { Deck, evaluateHand, evaluateOmahaHand, calculatePots, calculateBettingState,
    validateAction, calculateRake, determineWinners } from './PokerEngine.js';
import type {
    Card, HandStage, SeatPlayer, ActionType, GameVariant,
    HandConfig, GameState, ActionRecord, HandEvent, ShowdownResult,
    EvaluatedHand, Pot, Winner, RakeConfig,
} from '../types.js';
...
export class HandController {
    ...
    private getCardsPerPlayer(): number {
        switch (this.config.gameVariant) {
            case 'plo': case 'plo4': return 4;
            case 'plo5': return 5;
            case 'plo6': return 6;
            case 'plo8': return 4;
            case 'ofc': case 'ofc_pineapple': return 5;
            default: return 2;
        }
    }
    ...
    private completeHand(): void {
        ...
        if (activePlayers.length > 1) {
            const evaluator = this.config.gameVariant.startsWith('plo') ? evaluateOmahaHand : evaluateHand;
