...
import type {
    Card, CardRank, CardSuit, HandStage, SeatPlayer, ActionType,
    EvaluatedHand, Pot, BettingState, RakeConfig, Winner,
} from '../types.js';
...
function evaluate5Cards(cards: Card[]): EvaluatedHand {
    ...
    if (counts[0] === 4) return { ranking: HAND_RANKINGS.FOUR_OF_A_KIND, name: 'Four of a Kind', cards: sorted, kickers: getKickers(rankCounts) };
    if (counts[0] === 3 && counts[1] === 2) return { ranking: HAND_RANKINGS.FULL_HOUSE, name: 'Full House', cards: sorted, kickers: getKickers(rankCounts) };
    if (isFlush) return { ranking: HAND_RANKINGS.FLUSH, name: 'Flush', cards: sorted, kickers: ranks };
    if (isStraight) return { ranking: HAND_RANKINGS.STRAIGHT, name: 'Straight', cards: sorted, kickers: isWheel ? [5, 4, 3, 2, 1] : ranks };
    if (counts[0] === 3) return { ranking: HAND_RANKINGS.THREE_OF_A_KIND, name: 'Three of a Kind', cards: sorted, kickers: getKickers(rankCounts) };
    if (counts[0] === 2 && counts[1] === 2) return { ranking: HAND_RANKINGS.TWO_PAIR, name: 'Two Pair', cards: sorted, kickers: getKickers(rankCounts) };
    if (counts[0] === 2) return { ranking: HAND_RANKINGS.PAIR, name: 'Pair', cards: sorted, kickers: getKickers(rankCounts) };
    ...
    for (const hole of holeCombos) {
        for (const board of boardCombos) {
            ...
            if (!bestLow || compareLowHands(sortedRanks, bestLowRanks) < 0) {
                bestLow = { ranking: 0, name: `Low: ${sortedRanks.join('-')}`, cards: fiveCards, kickers: sortedRanks };
...
export function validateAction(
    action: ActionType, amount: number | undefined, playerStack: number, bettingState: BettingState
): { valid: boolean; error?: string } {
    const { currentBet, minRaise, toCall } = bettingState;

    switch (action) {
        case 'fold': return { valid: true };
        case 'check':
        ...
        default: return { valid: false, error: 'Invalid action' };
    }
}
...
export function determineWinners(
    players: SeatPlayer[], communityCards: Card[], pots: Pot[], gameVariant: string = 'nlh'
): Winner[] {
